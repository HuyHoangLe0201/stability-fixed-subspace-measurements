...
\end{aligned}
\end{equation}
Angles are in radians. Define $v_\theta=(\cos\theta,\sin\theta)^{\top}$ and its projector $P_\theta=v_\theta v_\theta^{\top}$, and let $\theta_\gamma$ be a maximizing measurement angle. The leading eigenvector of $S_2$ has angle $\theta_0=\CoreAngle$ and eigengap $\Delta=\CoreGap$. Let the random angle $\alpha$ take value $\alpha_j$ with probability $p_j$, set $x=\cos(\alpha-\theta_0)$ and $y=\sin(\alpha-\theta_0)$, and define the fourth-order contraction $A=\EE[x^3y]=\CoreA$. For Gaussian input, \eqref{eq:tilt} predicts $(\theta_\gamma-\theta_0)/\gamma\to-A/\Delta=\CoreTilt$. Consequently,
\begin{align}
...
\begin{figure}[t]
\centering\includegraphics[width=0.95\textwidth]{figures/fig_optimizer_stability.pdf}
...
\begin{figure}[t]
\centering\includegraphics[width=0.95\textwidth]{figures/fig_orientation.pdf}
...
\begin{figure}[t]
\centering\includegraphics[width=0.95\textwidth]{figures/fig_surrogate.pdf}
