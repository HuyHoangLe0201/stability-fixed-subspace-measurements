...
\emph{(a) Approximation of objectives and optimal values.} Suppose, at a fixed rank $K$,
\begin{equation}
\sup_{P\in\Pset}|M_n(P;\mu)-M_n(P;\nu)|\le\varepsilon_n,
\quad 1\le n\le r,
\end{equation}
where $\varepsilon_n\ge0$ are moment-error bounds. With the coefficients $b_n$ of Lemma~\ref{lem:expansion}, define
\begin{equation}
...
\emph{(c) Finite directional representation.} For every law $\mu$, there is a weighted law
\begin{equation}
\nu_r=\sum_{\ell=1}^m w_\ell\delta_{u_\ell},\qquad
w_\ell>0,\quad\sum_{\ell=1}^m w_\ell=1,
\end{equation}
where $\delta_{u_\ell}$ denotes unit point mass and every node $u_\ell$ belongs to $\operatorname{supp}\mu$, the smallest closed set of full $\mu$-measure, such that
\begin{equation}
...
The next result closes the gap between a known directional law and a law learned
from finitely many directions. It uses the empirical law
$\widehat\mu_N=N^{-1}\sum_{i=1}^N\delta_{U_i}$ and is uniform over the continuous
projector class, rather than over a finite numerical candidate set.
...
suppose $h_R\in C^{r+1}([0,\gamma])$. Define
\begin{equation}
L_\gamma=\sup_{0\le s\le\gamma}|h_R'(s)|,
\qquad
...
\begin{equation}
\rho_{r+1}(c)=\frac{(-1)^r}{2}\int_0^{\gamma c}\frac{t^r}{1+t}\,dt.
