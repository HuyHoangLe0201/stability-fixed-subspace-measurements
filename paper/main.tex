...
\input{results.tex}
\input{core_results.tex}
\input{rank_results.tex}
\begin{document}
...
This reduction is the organizing idea of the paper. At small SNR, the first
term of $h_R(\gamma C_P(U))$ depends only on the average coverage, so the
directional second moment selects a baseline subspace. At finite SNR,
higher coverage moments can move the best subspace. If the directional law
is estimated or compressed, a bound that holds for every candidate subspace
lets us transfer a design without knowing its optimizer in advance.
...
For the three-dimensional plots below, parameterize a unit measurement
direction by $v=(\sin\theta\cos\phi,\sin\theta\sin\phi,\cos\theta)^\top$
and write $P_v=vv^\top$. Here $\theta$ is the polar angle and $\phi$ the
azimuthal angle; the plotted angles are in degrees.
...
information landscapes behind Proposition~\ref{prop:examples}. At Gaussian SNR $\gamma=3$, a uniform ensemble of
...
\label{eq:haar_high}
\end{equation}
Define the Haar-law information loss by
$\Lloss_{\rm Haar}(\gamma)=\tfrac12\log(1+\gamma)-\Bbase_{d,K}(\gamma)$.
...
\label{eq:digamma}
\end{equation}
To justify the limit, write the difference from $\tfrac12\log\gamma$ as $\tfrac12\EE\log(C+1/\gamma)$. For $\gamma\ge1$, the absolute value is bounded by $|\log C|+\log2$, which is integrable under the Beta law. Dominated convergence applies.

\input{numerical_revision.tex}
